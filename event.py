...
class Event():
    ...
    def do_suitcase(self, df_alarm, df_issue, df_receipt, df, num_suitcase,Receipt,df_output):
        PT=[]
        ...
        # df_output.at[0, "size_issue"]=len(df_issue.index)
        # df_output.at[0, "size_alarm"]=len(df_alarm.index)
        # df_output.at[0, "size_receipt"]=len(df_receipt.index)
        # df_output.at[0,"quntity_package"]="None"
        if df_receipt_.empty:
            pass
        else:
            Receipt.check_receipt_period(df_receipt_,df,df_output) #have not worked yet
        ...
        print("cnt_alarm",len(df_alarm.index))
        print("cnt_issue",len(df_issue.index))
        print("cnt_receipt",len(df_receipt.index))
        ...
        # print("df_receipt_before",len(df_receipt.index) )
        
       
        # df_issue.drop(index=df_issue_.index, inplace=True)
        
        # df_alarm.drop(index=df_alarm_.index, inplace=True)
        
        # df_receipt.drop(index=df_receipt_.index, inplace=True) #this is working!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def period_time(self): #it will be beter use SQL query 'BETWEEN' and then use only one dataframe?????
        ...
        data_analyz=pd.read_sql_query("SELECT * FROM polycomm_suitcase WHERE DATE(local_date) > %(date)s order by local_date limit 1000" ,con=conn, params={"date": self.date_df})
        conn.commit()
        conn.close()
        return data_analyz
