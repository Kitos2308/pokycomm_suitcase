...
class Issue(Event):
    ...
    def check_issue(self, df, number_row):
        ...
        data2=pd.read_sql_query("SELECT * FROM polycommissue WHERE date BETWEEN  %(dstart)s AND %(dfinish)s ",con=conn, params={"dstart":start_date, "dfinish":end_date })
        if data2.empty : 
            conn.commit()
            conn.close() 
            """ print("NUMBER_ROW:")
            ...
            """ print("NUMBER_ROW3000:")
            print(number_row)
            print(data2)
            print(start_date)
            print(end_date)
            print("df")
            print(df.loc[number_row,:])  """
            conn.commit()
            conn.close()
            return 1

    def take_data_issue(self):
        
        conn=psycopg2.connect("dbname='unpack' user='postgres' password='root' host='localhost' port='5432' ") 
        data_issue=pd.read_sql_query("SELECT * FROM polycommissue WHERE localdate >%(date)s order by localdate limit 1000" ,con=conn, params={"date":self.date_df})
        conn.commit()
        conn.close()
        return data_issue
